...
const TotalProperty = () => {
  ...
  const getOwnBlog = async () => {
    try {
      const res = await axios.get(
        `https://balanza-backend-8dr2.onrender.com/api/v1/blog/get-own-blogs`,
  ...
  const getTotalComments = async () => {
    try {
      const res = await axios.get(
        `https://balanza-backend-8dr2.onrender.com/api/v1/comment/my-blogs/comments`,
        { withCredentials: true }
      );
      if (res.data.success) {
  ...
  const getTotalLikes = async () => {
    try {
      const res = await axios.get(
        `https://balanza-backend-8dr2.onrender.com/api/v1/blog/my-blogs/likes`,
        { withCredentials: true }
      );
      if (res.data.success) {
